...
Embedding inputs are independent [`MessagePart`](https://github.com/WordPress/php-ai-client/blob/trunk/src/Messages/DTO/MessagePart.php) values, not a conversation. `embed()` and `withInput()` accept a string, `MessagePart`, `File`, or message-part array shape; `withInputs()` accepts a list of those inputs. Model selection accounts for their input modalities and embedding configuration.
